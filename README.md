...
* 全部AIエージェントに書かせたAIエージェント管理ツールです。皮肉なものです。
    * 実験作で自分が遊ぶためのものです。
    * まだ未完成です。色々荒いです。構成色々ぐちゃぐちゃな気がする。（多分完成させないです。）
    * 開発してる感がなかったり、とはいえAIエージェント時代の開発に慣れないとなと思ったり。
* やはりコンテキストウィンドウの制約が鬼門。
* エージェントの仕組みによるのだろうけど、直前の修正内容を明確に覚えていない。
    * ロールバックなどの処理はgitなどのバージョン管理を組み合わせないと危険。
    * エージェント側の仕組みを改善 or 細かいバージョン管理って必要だよねという話。
    * MCPでgit連携させて勝手にやらせる or 必ず一連のgitコマンドを打たせる？
        * ブランチ分けて勝手にやってくれれば汚れもしないから良いかなー？
